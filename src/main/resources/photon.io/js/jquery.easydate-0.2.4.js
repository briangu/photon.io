(function($)
{
    ...
    function __s(str, value, settings)
    {
      return __(str, value, settings) + "s";
    }
            ...
            if(diff_abs < unit.limit)
            {
                ...
                if(diff < 0)
                    ...
                    format_string = __("past_format", NaN, settings)
                        .replace("%s", __("", NaN, settings))
                if (unit.name == "h" || unit.name == "m" || unit.name == "s") {
                  return format_string
                      .replace("%t", val + "" + __(unit.name, val, settings));
                } else {
                  return format_string
                      .replace("%t", val + " " + __s(unit.name, val, settings));
                }
            }
        }
